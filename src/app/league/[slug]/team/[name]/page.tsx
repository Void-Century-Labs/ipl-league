...
async function getMemberWithPlayers(leagueId: string, memberName: string) {
...
export default async function TeamPage({
  ...
  const data = await getMemberWithPlayers(league.id, memberName);

  if (!data) {
    return (
      <div className="text-center py-16">
        <p className="text-lg text-gray-500">Team not found</p>
        <Link href={`/league/${slug}`} className="text-blue-500 hover:underline text-sm mt-2 inline-block">
          Back to leaderboard
        </Link>
      </div>
    );
